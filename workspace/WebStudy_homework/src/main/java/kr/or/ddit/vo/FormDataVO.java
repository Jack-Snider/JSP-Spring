...
/**
 * VO (Value Object), DTO (DataTransferObject), JavaBean, Model
 * 1.   값을 가질수 있는 property 정의.
 * 2.   property 캡슐화 (은닉화, 정해진 절차에 따라서만 접근 가능하게 한다.)
 * 3.   일정한 절차에 따라 값을 변경할 수 있는 interface(절차) 메소드 제공.
 *       getter/setter
...
public class FormDataVO implements Serializable{
   ...
   // 노출되면 안되는 정보이기 때문에 toString에서 제외한다.
   // transient : 직렬화에 포함되지 않게 해준다.
